...
public:
    ...
    std::size_t raw_index( const position_t& pos ) const {
        return index( pos );
    }

    const pheronome_t& buffer_cell( std::size_t idx ) const {
        return m_buffer_pheronome[idx];
    }

    void set_buffer_cell( std::size_t idx, const pheronome_t& value ) {
        m_buffer_pheronome[idx] = value;
    }

    const pheronome_t* raw_map_cells() const {
        return m_map_of_pheronome.data();
    }

    pheronome_t* raw_buffer_cells() {
        return m_buffer_pheronome.data();
    }
...
#pragma omp parallel for collapse(2) schedule(static)
    ...
    pheronome_t compute_mark( const position_t& pos ) const {
        long i = pos.x;
        long j = pos.y;
        assert( i >= 0 );
        assert( j >= 0 );
    ...
    void mark_pheronome( const position_t& pos ) {
        m_buffer_pheronome[index( pos )] = compute_mark( pos );
    }
